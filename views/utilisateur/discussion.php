                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                <div class="form-bar">
                                    <div class="d-flex">
                                        <div class="mx-2">
                                            <img src="../../publics/images/utilisateur/<?= $admin->photo ?>" class="rounded-circle img-circle-banner me-2">
                                        </div>
                                        <div>
                                            <p class="banner-text"> <?= $admin->nom ?> <br><?= $admin->service ?></p>
                                        </div>
                                    </div>
                                ...
                                <div class="container pane-message">
                                    <div class="message p-2">
                                        <?php
                                        $admin_id = $admin->id;
                                        $execution = afficher_message($connexion, $id, $admin_id)
                                        ?>
                                        <?php while ($messages = $execution->fetch(PDO::FETCH_OBJ)) { ?>
                                            <?php if ($messages->destinataire === $admin_id) { ?>
                                ...
                                <form action="../../controllers/utilisateur/discussion.php?id=<?= $admin->id ?>" method="post" class="d-flex">
                                    <textarea name="message" id="input" class="form-control" rows="3" required="required" placeholder="Contacter l'administrateur"></textarea>
                                    <button class="btn btn-outline-success bout" type="submit" name="envoyer_admin"> <i class="fa fa-paper-plane" aria-hidden="true"></i></button>
                                </form>
                            </div>
                        </div>
                    </div>
